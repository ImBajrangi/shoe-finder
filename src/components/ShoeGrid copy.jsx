...
function ShoeTile({
    ...
    useFrame((state, delta) => {
        ...
        const targetFilterScale = matchesFilter ? 1 : 0.8;
        easing.damp(
            filterOpacity,
            "current",
            targetFilterOpacity,
        ...
        // Sleep check: If filtered out and invisible, stop processing
        if (filterOpacity.current < 0.01 && !matchesFilter) {
        ...
        if (gridVisible) {
            ...
            const targetBreath = isActiveItem
                ? 1 +
                Math.sin(state.clock.elapsedTime * 2.0) * 0.015
                : 1;
            easing.damp(
                breathScale,
                "current",
...
export default function ShoeGrid() {
    ...
    // Voice command handler - parses AI responses for action phrases
